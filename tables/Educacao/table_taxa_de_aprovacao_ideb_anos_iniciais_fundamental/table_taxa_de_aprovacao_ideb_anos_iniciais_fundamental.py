...
table_name = 'table_taxa_de_aprovacao_ideb_anos_iniciais_fundamental'
...
def dataframe():
    for ano in range(ultimo_ano, ano_atual):
        ...
        WHERE ano = {ano}  and ensino = 'fundamental'  and anos_escolares = 'iniciais (1-5)'
        """
        ...
        rename = {
            ...
            'indicador_rendimento': 'Indicador de Rendimento (P) - Anos Iniciais',
            'ano': 'Ano'
        }

        df = df.rename(columns=rename)[['codmun','Rede', 'Taxa de Aprovação','Indicador de Rendimento (P) - Anos Iniciais', 'Ano']]
...
def run_table_taxa_de_aprovacao_ideb_anos_iniciais_fundamental():
    try:
        dataframe()
